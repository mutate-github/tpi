...
ash [ event [all] (EventName) | sess (SID SERIAL# [all|nosqlid] | SQL_ID) | prg (PROGRAM) | sql (SQL_ID|SQL_TEXT) | plan SQL_ID [format display plan] | sqlstat SQL_ID ] - Top Event, Sessions, SQL V$ACTIVE_SESSION_HISTORY in last time
dhash [hours - def3] [ event [all] (EventName) | sess (SID SERIAL# [all|nosqlid] | SQL_ID) | prg (PROGRAM) | sql (SQL_ID|SQL_TEXT) | plan SQL_ID [format display plan] | sqlstat SQL_ID | growseg [SEGMENT] ] - Top Events, Sessions, SQL from DBA_HIST_ACTIVE_SESS_HISTORY
